...
#Springer
ninja = Actor('springer-1',(500,250))
ninja_x_velocity = 0
ninja_y_velocity = 0
gravity = 1
jumping = False
...
gem = Actor('diamant',(diamond_x[d_xy],diamond_y[d_xy]))
points = 0
...
def ninja_move():
    ...
    if not collidecheck():
        ...
        if len(timer) > 5 and not jumped:
            allowx = False
            ninja.image = 'springer-hoch'
            if len(timer) > 20:
                ninja.image = 'springer-runter'
                if len(timer) > 30:
                    ninja.image = 'springer-runter2'

    #Links/rechts Bewegung
    if (keyboard.left) and allowx:
        if (ninja.x > 40) and (ninja_x_velocity > -8):
            ninja_x_velocity -= 2
            ninja.image = "springer-links"
            if (keyboard.left) and jumped:
                ninja.image = "springer-slinks"
    if (keyboard.right) and allowx:
        if (ninja.x < 960) and (ninja_x_velocity < 8):
            ninja_x_velocity += 2
            ninja.image = "springer-rechts"
            if (keyboard.right) and jumped:
                ninja.image = "springer-srechts"
